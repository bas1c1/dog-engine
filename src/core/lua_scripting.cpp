...
void dlua::init() {
	...
	sol::usertype<camera> camera_type = lua.new_usertype<camera>("camera", sol::constructors<camera(), camera(int, int, vec2d, WFSGL&)>());
	...
	sol::usertype<transform> transform_type = lua.new_usertype<transform>("transform", sol::constructors<transform(), transform(int, int, vec2d, WFSGL&, ObjectType, vec2d, float)>());
	...
	sol::usertype<sprite> sprite_type = lua.new_usertype<sprite>("sprite", sol::constructors<sprite(), sprite(int, int, camera&), sprite(int, int, std::vector<Pixel>, camera&), sprite(std::string, camera&)>());
	sprite_type["scale"] = sol::property(&sprite::getScale, &sprite::setScale);
	sprite_type["setScaleAaa"] = &sprite::setScale;
