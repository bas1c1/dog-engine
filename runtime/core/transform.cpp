...
transform::transform(int width, int height, vec2d pos, ObjectType objType, vec2d localScale, float rotationAngle) {
...
transform::transform(int width, int height, int posx, int posy, ObjectType objType) {
	pos = vec2d(posx, posy);
	this->objType = objType;
	switch (objType) {
	case ObjectType::BASE:
...
ObjectType transform::getObjType() { return objType; }
...
void transform::setObjType(ObjectType objType) {
	this->objType = objType;
	switch (objType) {
	case ObjectType::BASE:
		this->pos = pos - vec2d(width / 2, height / 2);
